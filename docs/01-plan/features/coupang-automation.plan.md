...
- [ ] [Do] 발주 자동화 로직 구현 (Playwright + Excel)
  ...
  - [x] 상품정보 시트 A열 운영여부 추가 (사용자 수동 입력: 운영/미운영), 전체 열 B~R로 이동
  - [x] 상품정보 시트에 발주 계산 컬럼 추가 (M~S열: 운영 가능일, 30일/7일 기준 재고, 최종 발주 참고 수량, 최종 발주량, 최종 발주량 BOX, 사용자 확정)
  - [x] n8n 열 참조 업데이트 (재고 전체 처리: C:G, 상품정보 업데이트: D2:D, H2:H, I1, I~S 11열 출력)
  - [x] 발주수량 자동 계산 (max(30일기준, 7일기준) - 현재고, 입수량 단위 올림)
  - [x] 운영가능일 ≥ 30이면 Q·R열 "발주X", S열 0 처리
  - [x] S열(사용자 확정) 추가: 초기값 R열 복사, 사용자 수정 후 Playwright 발주 시 사용
