...
| **L** | **운영 가능일** | **n8n 자동** | **품절 예상일 - 오늘 (숫자만, 단위 없음)** |
| **M** | **최근 30일 기준 재고** | **n8n 자동** | **H열 × 30 (30일치 재고량)** |
| **N** | **최근 7일 기준 재고** | **n8n 자동** | **I열 × 30 (7일 트렌드 기반 30일치)** |
| **O** | **최종 발주 참고 수량** | **n8n 자동** | **max(M열, N열)** |
| **P** | **최종 발주량** | **n8n 자동** | **ceil((O열 - J열) / G열) × G열, 0 이하면 0** |
| **Q** | **최종 발주량 (BOX)** | **n8n 자동** | **P열 ÷ G열 (박스 수)** |
...
[1] 구글 시트에서 발주 대상 계산 (n8n 자동 계산)
    ...
    → L열: 운영 가능일(숫자), P열 > 0인 상품이 발주 대상, Q열: 박스 수
            ↓
...
| 상품정보 | - | PUT 덮어쓰기 (서비스 계정 JWT) | SKU ID (C열 기준) | H~Q (일평균, 재고, 품절예상일, 운영가능일, 기준재고, 발주참고, 발주량, BOX) |
